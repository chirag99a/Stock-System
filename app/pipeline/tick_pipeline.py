...
async def process_tick_pipeline(security_id: str, ltp: float, ts: str, source: str) -> None:
    ...
    """
    # Monotonic start to end when decision is emitted.
    start_ns = None
    try:
        from app.metrics.latency import get_tick_start_ns
    ...
    # Measure tick-to-signal latency (decoder emitted decision here)
    if start_ns is not None:
        await record_tick_to_signal_latency(start_ns=start_ns, decision_ns_start=None, decision="signal_emitted")
    ...
    await enqueue_notification_for_trade(trade_id=trade_id, dedup_key=trade.notification_dedup_key, message_template=trade)
